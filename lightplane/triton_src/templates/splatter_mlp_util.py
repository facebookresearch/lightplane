...
# cog.outl(f"   w = tl.reshape(tl.load(ptr + offs), (dim_in, dim_out))")
# cog.outl(f"   return w")
# cog.outl("")
# cog.outl("@triton.jit")
# cog.outl("def load_bias(ptr, dim, BLOCK_SIZE):")
# cog.outl("    return tl.reshape(tl.load((ptr + tl.arange(0, dim))[None, :] + tl.zeros((BLOCK_SIZE, 1), dtype=tl.int32)), (BLOCK_SIZE, dim))")
# cog.outl("")
# cog.outl("@triton.jit")
