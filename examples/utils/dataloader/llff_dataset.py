...
class LLFFDataset(DatasetBase):
    ...
    def __init__(
        ...
        hold_every=8,
        offset=250,
        sample_image: bool = False,  # whether sample image or rays
        **kwargs,
    ):
        super().__init__()
        if scale is None:
    ...
    def gen_rays(self, factor=1):
        ...
        # normalize
        origins = origins - torch.tensor(self.scene_center)[None, :]
        dirs = dirs / torch.tensor(self.scene_radius)[None, :]
        origins = origins / torch.tensor(self.scene_radius)[None, :]
    ...
    def selectDepth(self, dmin, dmax, offset):
        ...
        if self.dmin < 0 or self.dmax < 0:
            if os.path.exists(self.dataset + "/bounds.txt"):
                with open(self.dataset + "/bounds.txt") as fi:
                    data = [
                ...
                with open(self.dataset + "/planes.txt") as fi:
        ...
        if dmin > 0:
            print(f"Overriding dmin {self.dmin:f}-> {dmin:f}")
            self.dmin = dmin
        if dmax > 0:
            print(f"Overriding dmax {self.dmax:f}-> {dmax:f}")
            self.dmax = dmax
        if offset != 200:
...
def readCameraDeepview(dataset):
    cams = {}
    imgs = {}
    with open(os.path.join(dataset, "models.json")) as fi:
        js = json.load(fi)
