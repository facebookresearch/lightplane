...
"""
Based on ARF's code: https://github.com/Kai-46/ARF-svox2/blob/master/opt/nnfm_loss.py
"""
import torch
import torchvision
...
def match_colors_for_image_set(image_set, style_img):
    ...
    cov_c = torch.matmul((image_set - mu_c).transpose(1, 0), image_set - mu_c) / float(
        image_set.size(0)
    )
    cov_s = torch.matmul((style_img - mu_s).transpose(1, 0), style_img - mu_s) / float(
        style_img.size(0)
    )
...
class NNFMLoss(torch.nn.Module):
    ...
    def forward(
        ...
        loss_dict = {x: 0.0 for x in loss_names}
        for block in blocks:
            ...
            if "gram_loss" in loss_names:
                loss_dict["gram_loss"] += torch.mean(
                    (gram_matrix(x_feats) - gram_matrix(s_feats)) ** 2
                )

            if "content_loss" in loss_names:
                content_feats = torch.cat(
                    [content_feats_all[ix_map[ix]] for ix in layers], 1
                )
...
if __name__ == "__main__":
    ...
    fake_image_set_new, color_tf = match_colors_for_image_set(
        fake_image_set, fake_style
    )
